...
function initScrub(cfg) {
  ...
  for (let i = 0; i < cfg.frameCount; i++) {
    const img = new Image();
    img.decoding = "async";
    // Prioritise frame 0 so the hero paints fast; let the rest trickle in
    // instead of all 540 frames fighting over the connection at once.
    if ("fetchPriority" in img) img.fetchPriority = (i === 0) ? "high" : "low";
    img.onload = () => {
      // Paint frame 0 the instant it lands (no scroll required); refresh the
      // visible frame if it was the one that just finished decoding.
      if (i === 0 && current < 0) { current = 0; draw(0); }
      else if (i === current) { draw(current); }
    };
  ...
  function draw(index) {
    ...
    let dw, dh, dx, dy;
    // cover = fill + crop overflow; contain = fit whole image, letterbox in bg.
    const wider = (fit === "contain") ? (ir < cr) : (ir > cr);
    if (wider) { dh = ch; dw = ch * ir; dx = (cw - dw) / 2; dy = 0; }
    else       { dw = cw; dh = cw / ir; dx = 0; dy = (ch - dh) / 2; }
    ctx.fillStyle = bgFill; ctx.fillRect(0, 0, cw, ch);
    ctx.drawImage(img, dx, dy, dw, dh);
    canvas.classList.add("cine-ready");
  }
  function resize() {
    ...
    // Never flash black: lay down the warm section background immediately,
    // even before any frame has finished downloading.
